...
export const Route = createFileRoute("/_app/api-tester")({
  head: () => ({ meta: [{ title: "API Tester · DiscourseGuard" }] }),
  component: ApiTester,
});
...
function ApiTester() {
  ...
  const saveToQueue = () => {
    ...
    store.addMessage({
      ...
      action: result.warning === "high" ? "deleted" : result.warning === "medium" ? "warned" : "logged",
      reviewed: false,
    });
  ...
  const methodIcon = result?.method === "safe_phrase" ? Zap : result?.method === "keyword" ? Layers : Cpu;
  const MethodIcon = methodIcon;

  return (
    <div className="p-6 lg:p-10 max-w-6xl mx-auto space-y-6">
      <header>
        <div className="text-xs font-semibold text-primary uppercase tracking-widest">Developer</div>
        <h1 className="mt-2 text-3xl lg:text-4xl font-display font-bold">API Tester</h1>
        <p className="mt-1.5 text-sm text-muted-foreground">
          Send text through the three-layer pipeline. {settings.useMockApi
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        <Card className="lg:col-span-3 p-6 border-border/60 shadow-card-soft space-y-4">
          ...
          <div className="flex gap-2 pt-2">
            <Button onClick={submit} disabled={loading} className="bg-gradient-primary shadow-elegant">
        ...
        <Card className="lg:col-span-2 p-6 border-border/60 shadow-card-soft">
          <div className="flex items-center justify-between mb-3">
            ...
            {latency !== null && (
              <Badge variant="outline" className="font-mono text-[10px]">{latency}ms</Badge>
            )}
          </div>
          {!result && (
          ...
          {result && (
            <div className="space-y-4">
              ...
              <div>
                <div className="text-[11px] uppercase tracking-widest text-muted-foreground">Prediction</div>
              ...
              <div>
                <div className="text-[11px] uppercase tracking-widest text-muted-foreground mb-2">Confidence</div>
      ...
      {result && (
        <Card className="p-0 border-border/60 shadow-card-soft overflow-hidden">
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-border/60 bg-muted/40">
            <div className="text-xs font-mono text-muted-foreground">POST {settings.useMockApi ? "mock://classify" : `${settings.apiUrl}/classify`}</div>
...
{JSON.stringify(result, null, 2)}
      ...
      <Card className="p-6 border-border/60 shadow-card-soft">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {[
            { i: Zap,    t: "1 · Safe-phrase override", d: "Whitelist idioms like \"kill it with fire\" return safe immediately." },
            { i: Layers, t: "2 · Keyword rule layer",   d: "Curated hate/offensive terms — fast and interpretable." },
            { i: Cpu,    t: "3 · ML model",             d: "Logistic Regression + SMOTE returns per-class confidence." },
          ].map(({ i: Icon, t, d }) => (
